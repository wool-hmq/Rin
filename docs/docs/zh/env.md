...
| `SMTP_MAIL` | SMTP 发件邮箱 | SMTP 服务商处获取 |
| `SMTP_USER` | SMTP 登录用户名 | SMTP 服务商处获取 |
| `SMTP_PASS` | SMTP 登录密码 | SMTP 服务商处获取 |
| `SMTP_HOST` | SMTP 发件地址（带端口） | 如 `smtp.example.com:465` 或邮件服务 HTTP API 地址 |
